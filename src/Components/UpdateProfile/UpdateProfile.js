...
export const UpdateProfile = () => {
  ...
  const [photoURL, setPhotoURL] = useState('');
    ...
    const fetchUserProfile = async () => {
      try {
        const res = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyBLiObmhILV7-mxeT4ePopP3ibfV4wP6no`, {
          method: 'POST',
          body: JSON.stringify({
            idToken: token,
          }),
    ...
    const profileSubmitHandler=async(event)=>{
        ...
        try{
       const res = await fetch('https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyBLiObmhILV7-mxeT4ePopP3ibfV4wP6no',{
        method : 'POST',
        body :JSON.stringify({
            idToken: token,
